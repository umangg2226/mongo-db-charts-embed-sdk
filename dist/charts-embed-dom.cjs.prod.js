...
var _isEqual = _interopDefault(require('lodash/isEqual')),
  _isEmpty = _interopDefault(require('lodash/isEmpty')),
  bson = require('bson-m-react18'),
  chatty = require('@looker/chatty')
...
const getChartUrl = (options) => {
  ...
  parseCSSMeasurement = (value) =>
    'string' == typeof value
      ? value
      : 'number' == typeof value
      ? `${value}px`
      : null,
  getBackground = (background, theme) =>
    'string' == typeof background && background.length > 0
      ? background
      : 'dark' === theme
      ? '#21313C'
      : '#FFFFFF',
  isJWTExpired = (jwt) => {
    try {
      const [header, payload, signature] = jwt.split('.'),
        { exp: exp } = JSON.parse(atob(payload))
...
async function getRealmUserToken(stitchAppClient) {
  ...
  if (isJWTExpired(client.auth.authInfo.accessToken))
    if (client.auth.refreshCustomData) await client.auth.refreshCustomData()
    else {
...
const getChartOptions = (options) => {
  ...
  if ('string' != typeof baseUrl || 0 === baseUrl.length)
    throw new Error('Base URL must be a valid URL')
  if ('string' != typeof chartId || 0 === chartId.length)
    throw new Error('Chart ID must be specified')
  if (void 0 !== width && !['number', 'string'].includes(typeof width))
    throw new Error('Width must be a string or number if specified')
  if (void 0 !== height && !['number', 'string'].includes(typeof height))
    throw new Error('Height must be a string or number if specified')
  if (void 0 !== filter && (!filter || 'object' != typeof filter))
    throw new Error('Filter must be an object if specified')
  if (void 0 !== refreshInterval && 'number' != typeof refreshInterval)
    throw new Error('refreshInterval interval must be a number if specified')
  if (void 0 !== autoRefresh && 'boolean' != typeof autoRefresh)
    throw new Error('autoRefresh must be a boolean if specified')
  if (void 0 !== maxDataAge && 'number' != typeof maxDataAge)
    throw new Error('maxDataAge must be a number if specified')
  if (void 0 !== theme && 'string' != typeof theme)
    throw new Error('Theme must be a string if specified')
  if (void 0 !== showAttribution && 'boolean' != typeof showAttribution)
    throw new Error('Attribution must be a boolean value if specified')
  if (void 0 !== getUserToken && 'function' != typeof getUserToken)
    throw new Error('getUserToken must be a function')
  if (void 0 !== background && 'string' != typeof background)
...
class Chart {
  ...
  _send(eventName, ...payload) {
    return this.connection
      ? this.connection.sendAndReceive(eventName, ...payload)
      : Promise.reject(
  ...
  async isAutoRefresh() {
    const [result] = await this._send('get', 'autorefresh')
    return 'number' == typeof result || 'boolean' == typeof result
      ? Boolean(result)
      : Promise.reject('unexpected response received from iframe')
  }
  ...
  async getFilter() {
    ...
    return 'object' == typeof result && null !== result
      ? result
      : Promise.reject('unexpected response received from iframe')
  }
  ...
  addEventListener(event, eventHandler, options) {
    ...
    if (
      (null !== (_h$options$includes = h.options.includes) &&
        void 0 !== _h$options$includes &&
      ...
      return (
        ...
        this._send('eventHandler', event, handlerId, h.options)
      )
    }
  ...
  removeEventListener(event, eventHandler, options) {
    ...
    return handlerId
      ? (delete handlers[handlerId],
        this._send('eventHandler', event, handlerId))
      : Promise.resolve()
  }
  async render(container) {
    ...
    const chartUrl = getChartUrl(this.options),
      embedRoot = createElement('div', {
        style: {
          position: 'relative',
          overflow: 'hidden',
      ...
      host = chatty.Chatty.createHost(chartUrl)
        ...
        .on('event', (event, payload, handlerIds) => {
          ...
          for (const id of handlerIds)
            try {
              var _handlers$id
              null === (_handlers$id = handlers[id]) ||
                void 0 === _handlers$id ||
                _handlers$id.handle(payload)
            } catch (error) {
              console.warn(
                `Error calling handler for event [${event}]: ${error}`
              )
            }
        })
        .build()
    ...
    ;(this.connection = connection),
      await this.retrieveAndSetToken(),
